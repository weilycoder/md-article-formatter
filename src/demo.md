...
同时，还会将数学函数替换为对应的 LaTeX 命令，例如：

+ $min$ 将被替换为 `\min`；$max$ 将被替换为 `\max`；$mex$ 将被替换为 `\operatorname{mex}`；
+ $gcd$ 将被替换为 `\gcd`；$lcm$ 将被替换为 `\operatorname{lcm}`；
+ $log$ 将被替换为 `\log`。

同样可以在设置中添加替换规则，但设计上规定被替换项必须为字母，替换项必须为 `\` 开头的命令。
